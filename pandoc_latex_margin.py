...
from panflute import RawBlock, MetaList, MetaInlines, \
    RawInline, debug, run_filter


# pylint:disable=line-too-long
def get_correct_margin(length):
    if re.match('^(-?([1-9][0-9]*)|0)(pt|mm|cm|in|ex|em|bp|pc|dd|cc|nd|nc|sp)$', length) is not None:  # noqa: E501
        return length
    debug('[WARNING] pandoc-latex-margin: ' + margin + ' is not a correct LaTeX margin; using 0pt')  # noqa: E501
    return '0pt'
    ...
    if doc.format in ['latex', 'beamer'] and elem.tag in ['Div', 'CodeBlock']:
        ...
        if left is not None or right is not None:
            ...
            if right is None:
                right = '0pt'
            return [
                RawBlock('\\begin{pandocchangemargin}{' + left + '}{' + right + '}', 'tex'),  # noqa: E501
                elem,
...
def prepare(doc):
    ...
    if isinstance(meta, list):
        ...
        for definition in meta:
            ...
            if isinstance(definition, dict) and 'classes' in definition and isinstance(definition['classes'], list):  # noqa: E501
...
def finalize(doc):
    ...
    # Convert header-includes to MetaList if necessary
        ...
        doc.metadata['header-includes'] = MetaList(doc.metadata['header-includes'])  # noqa: E501
